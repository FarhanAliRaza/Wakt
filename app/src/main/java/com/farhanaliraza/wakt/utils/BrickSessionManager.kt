...
class BrickSessionManager @Inject constructor(
    ...
    suspend fun startDurationSession(sessionId: Long): Boolean {
        try {
            ...
            com.farhanaliraza.wakt.services.BrickOverlayService.requestShowOverlay(context)
     ...
     * Note: Overlay permission no longer needed - uses TYPE_ACCESSIBILITY_OVERLAY via AccessibilityService
     */
    private fun startEnforcementServices() {
        try {
            // Start the BrickEnforcementService for notifications and monitoring
            com.farhanaliraza.wakt.services.BrickEnforcementService.start(context)
    ...
    private fun exitBrickMode() {
        try {
            ...
            com.farhanaliraza.wakt.services.BrickOverlayService.requestHideForSessionEnd()
