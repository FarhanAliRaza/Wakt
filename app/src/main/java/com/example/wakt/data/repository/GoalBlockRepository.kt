package com.example.wakt.data.repository
...
class GoalBlockRepository @Inject constructor(
    ...
    suspend fun addItemToGoal(goalId: Long, itemName: String, itemType: com.example.wakt.data.database.entity.BlockType, packageOrUrl: String): Long {
    ...
    suspend fun createGoalWithItems(
        goal: GoalBlock,
        items: List<Pair<String, com.example.wakt.data.database.entity.BlockType>>
    ): Long {
        if (items.isEmpty()) {
        ...
        val goalItems = items.map { (packageOrUrl, type) ->
            val itemName = if (type == com.example.wakt.data.database.entity.BlockType.WEBSITE) {
                packageOrUrl
            } else {
